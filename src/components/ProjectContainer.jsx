...
const ProjectContainer = () => {
    ...
    const setproject = async () => {
        try {
            setLoading(true);
            const response = await createPorject(projectName, organisation.id);
    ...
    const handleEditProject = async () => {
        try {
            const response = await updateProject(projectName, selectedProjectId);
        ...
        <div className='m-4 flex  justify-center md:justify-start  flex-wrap gap-2 gap-y-4   '>
            ...
            <div className="max-w-sm  md:min-h-[150px] min-h-[100px] md:min-w-[300px] min-w-[300px] p-6 mx-2 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 cursor-pointer" onClick={() => setOpenModal(true)}>
                <div className='flex flex-col items-center justify-center '>
                    <GoPlus className='md:w-16 md:h-16 w-8 h-8 dark:text-white  ' />
                    ...
                    <div style={{ borderLeft: `6px solid ${project.bgColor}` }} className={`flex justify-between  max-w-[300px] md:min-h-[150px] bg-gray-100 dark:bg-gray-700  min-h-[100px] md:min-w-[300px] min-w-[300px]  pl-3 pr-0 mx-2 dark:text-black border border-gray-200 rounded-md shadow-sm  dark:border-gray-700 cursor-pointer`} onClick={() => handleProjectCard(project)}>
                        <div href="#">
                            ...
                            {project.createdBy && <p className='text-bold text-gray-500   flex  justify-between items-center dark:text-white mt-2'><FaUser className='mr-2 ' />
                                Created By,<span className=' text-gray-500 dark:text-white font-light'>{" "+project.createdBy}</span></p>}
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                            <Dropdown className='w-[150px]  ' arrowIcon={false} label={<IoEllipsisVerticalSharp className=' w-7 h-7 mx-2 mt-4  hover:md:opacity-90 md:opacity-0  opacity-90   dark:text-white dark:bg-gray-500 rounded-md' />} inline>
                                <Dropdown.Item onClick={() => { dispatch(getSelectedProject(project)); setOpenEditModal(true); setProjectName(project.name); setSelectedProjectId(project.projectId) }} className='dark:bg-gray-800'>Edit</Dropdown.Item>
            ...
            <Modal className='pt-[200px]  md:pt-0' dismissible show={openModal} onClose={() => setOpenModal(false)}>
                ...
                <Modal.Footer className='flex justify-center'>
                    ...
                    <button className='bg-blue-500 dark:bg-gray-500 text-white p-2 px-4 rounded-lg'  onClick={() => setproject()} >{loading ? <div role="status">
                                <svg aria-hidden="true" className="w-9 h-6 mx-1.5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600 dark:fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" /><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" /></svg>
...
<button className='bg-blue-500 dark:bg-gray-500 text-white p-2 px-4 rounded-lg'  onClick={() => handleEditProject()} >{loading ? <div role="status">
                                <svg aria-hidden="true" className="w-9 h-6 mx-1.5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600 dark:fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" /><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" /></svg>
                                <span className="sr-only">Loading...</span>
                            </div> : "Edit"}</button>
                </Modal.Footer>
