...
const Login = () => {
    ...
    const [selectedCredentials, setSelectedCredentails] = useState(null);
    ...
    const demoCredentials = [
        {
            id: 1,
            emailId: "ownerdemo@gmail.com",
            password: "Owner@123",
            role: "Owner"
        },
        {
            id: 2,
            emailId: "managerdemo@gmail.com",
            password: "Manager@123",
            role: "Manager"
        },
        {
            id: 3,
            emailId: "supervisordemo@gmail.com",
    ...
    const handleCheckBox = (credentails) => {
        setSelectedCredentails(credentails)
        setEmail(credentails.emailId);
        setPassword(credentails.password);
    ...
    return (
        <div className='h-screen'>
            <div className={`md:flex    md:justify-around  dark:bg-gray-800    md:mt-[20vh]  md:pt-0 ${isLoginPage ? "mt-[10vh]" : "mt-[4vh]"} `}>
                    ...
                    <div className='flex flex-col justify-center items-center bg-stone-50 opacity-90  border-gray-200 dark:bg-gray-900 dark:border-gray-700 md:w-[450px] w-[350px]   md:m-2 m-1 md:p-10 p-6  rounded-md shadow-md   '>
                        ...
                        <form className="max-w-md mx-auto w-full">
                            ...
                            <div className='flex flex-col justify-center items-center mt-10'>
                                ...
                                <p className='dark:text-white mt-2'>{isLoginPage ? "New User, " : "Already have account, "}<a className='underline cursor-pointer text-customSky ' onClick={() => { setIsLoginPage(!isLoginPage), setEmail(""); setPassword(""); setConfirmPassword(""); setFirstName(""); setLastName(""); setGroup(""); dispatch(removeErrorList()) }}>{isLoginPage ? "Sign Up" : "Log In"}</a></p>
                            </div>

                            {isLoginPage && <div className='mt-4'>
                                <p className='dark:text-white'>Demo Credentials, <span className='text-customSky cursor-pointer underline' onClick={() => { setShowCredentials(!showCredentials) }}>{showCredentials ? "close" : "show"}</span></p>
                                {showCredentials &&
                                    (demoCredentials.map((credentails) =>
                                    (
                                        <div key={credentails.id} className='mt-3'>
                                            <div className="flex items-center mb-4">
                                                <input id="default-checkbox" type="checkbox" checked={selectedCredentials && selectedCredentials.id === credentails.id} onChange={() => handleCheckBox(credentails)} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                                <label htmlFor="default-checkbox" className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{credentails.role}</label>
                                            </div>
                                        </div>
                                    )))
                                }
