            ...
            <Sidebar className={`${isSideBar ? "w-64" : "md:w-6 w-0"} rounded-lg duration-200  md:h-full overflow-y-hidden shadow-lg  bg-white   relative  h-full z-10 `}   >
                ...
                {isSideBar && <Sidebar.Items>
                    ...
                    <Sidebar.ItemGroup>
                        ...
                        <Sidebar.Item className="" href="/project" icon={HiHome}>
                            Home
                        </Sidebar.Item>
                        ...
                        {!isDisabled && <Sidebar.Collapse className='md:hidden' icon={HiOutlineClipboardList} label="Project">
                            ...
                            (
                                ...
                                <Sidebar.Item className="w-[200px] mx-1 md:hidden" onClick={() => { handleSetDefaultProject(project); dispatch(setSideBar(false)) }} key={project.id}>{project.name}</Sidebar.Item>
                        ...
                        {!isDisabled && <Sidebar.Item href="/project/task" icon={HiOutlineViewGrid}>
                            Tasks
                        </Sidebar.Item>}

                        {(!isDisabled && userPermissions) && userPermissions.includes("view_contractor") && <Sidebar.Item href="/project/contractor" icon={HiBriefcase}>
